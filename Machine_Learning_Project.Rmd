...
###Decision Tree
After setting the seed and training a tree with cross validation, the result is a tree with only 48.85% accuracy (note no 'D' is classified in any node). 
...
treeMatrix <- confusionMatrix(predFitTree, validation$classe)$overall
```
...
###Random Forest
Before settling on AdaBoost as the winning model, I train and run a random forest, resulting in the best off-the-bat accuracy of the three (99.2% on training set, 99.4% on validation set). It is run after setting up a cluster of CPU cores to enable parallel processing to reduce compute time.
...
##Out of Sample Error
I used k-fold cross-validation with k=10 and number of repeats equal to three when training the random forest. Thanks to the cross-validation, that out-of-box error rate estimate of 0.72% is a good estimate of the model's out-of-sample error rate. Additionally, by running that model on the validation set (which had **not** been used for training), we get a true out-of-sample error estimate of `r percent(1 - rfMatrix$overall[[1]])` (1 - highest accuracy):
